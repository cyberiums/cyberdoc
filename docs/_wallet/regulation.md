...
developer tool. The focus of this guide is creating a public list for the Cyberium mainnet
...
```shell
go get -u github.com/Cyberiums/cyberium/cmd/devp2p
```

To create a signing key, you might also need the `ethkey` utility.

```shell
go get -u github.com/Cyberiums/cyberium/cmd/ethkey
```
...
Then, to create the output set containing Cyberium mainnet nodes only, run
...
* `-eth-network ( mainnet | ropsten | rinkeby | goerli )` selects an Cyberium network.
