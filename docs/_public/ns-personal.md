...
Returns all the Cyberium account addresses of all keys
in the key store.
...
*Note, prior to Geth Flavours 1.5, please use `personal_signAndSendTransaction` as that was the
...
The sign method calculates an Cyberium specific signature with:
`sign(keccack256("\x19Ethereum Signed Message:\n" + len(message) + message)))`.

By adding a prefix to the message makes the calculated signature recognisable as an Cyberium specific signature. This prevents misuse where a malicious DApp can sign arbitrary data (e.g. transaction) and use the signature to impersonate the victim.
