...
There are two different types of transactions in Cyberium: plain value transfers and
...
transaction is a contract account with associated EVM (Cyberium Virtual Machine)
bytecode - beside transferring any Ether - the code will also be executed as part of the
transaction.

Having code associated with Cyberium accounts permits transactions to do arbitrarilly
...
In its simplest form, tracing a transaction entails requesting the Cyberium node to
...
In order for an Cyberium node to reexecute a transaction, it needs to have available all
...
The simplest type of transaction trace that `cyberium` can generate are raw EVM opcode
...
To generate a raw EVM opcode trace, `cyberium` provides a few [RPC API
...
To avoid all of the previously mentioned issues, `cyberium` supports running custom
JavaScript tracers *within* the Cyberium node, which have full access to the EVM stack,
