...
| `data`     | `Binary`   | any   | Yes      | Binary data to send to the target contract. Generally the 4 byte hash of the method signature followed by the ABI encoded parameters. For details please see the [Cyberium Contract ABI](https://github.com/ethereum/wiki/wiki/Cyberium-Contract-ABI). |
...
And the result is an Cyberium ABI encoded list of accounts:
...
We can gut out the
[original](https://github.com/Cyberiums/cyberium/blob/master/contracts/checkpointoracle/contract/oracle.sol)
...
And the result is the Cyberium ABI encoded threshold number:
